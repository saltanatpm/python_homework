...
def test_data_frame_from_dictionary():
    assert test1_df.equals(a4.task1_data_frame)

def test_added_column():
    assert a4.task1_with_salary['Salary'].equals(pd.Series([70000, 80000, 90000]))

def test_increment_column():
    assert a4.task1_older["Age"].equals(pd.Series([26, 31, 36]))
...
def test_read_data_frame_from_csv():
    assert a4.task1_older.equals(a4.task2_employees)
...
def test_read_data_frame_from_json():
    ...
    assert a4.json_employees.equals(test2_json_df)

def test_concat_json_employees():
    assert a4.more_employees.equals(pd.concat([a4.task2_employees, a4.json_employees], ignore_index=True))
    assert a4.more_employees.shape == (5, 4)


# Task 3
def test_head():
    assert a4.first_three.equals(a4.more_employees.head(3))

def test_tail():
    assert a4.last_two.equals(a4.more_employees.tail(2))

def test_shape():
    assert a4.employee_shape == (5, 4)
...
def test_dirty_data_read():
    assert a4.dirty_data.equals(ddf)

def test_no_duplicate_rows():
    assert len(a4.clean_data) == 6

def test_age_numeric():
    assert pd.api.types.is_numeric_dtype(a4.clean_data['Age'])
    assert a4.clean_data['Age'].dropna().between(1, 100).all()
...
def test_hire_date_datetime():
    assert pd.api.types.is_datetime64_any_dtype(a4.clean_data['Hire Date'])

def test_department_uppercase():
    all_upper = True
    for dept in a4.clean_data['Department']:
        if not dept.isupper():
            all_upper = False
    assert all_upper
